...
namespace complete_immediately {
  ...
  namespace txn {
    ...
    inline auto make_start_txn(cope::msg_t&& msg, state_t&& state) {
      return cope::txn::make_start_txn<state_t>(kTxnId,
    ...
    auto handler() -> cope::txn::handler_t {
      ...
      while (true) {
        auto& promise = co_await cope::txn::receive{ kTxnId, state };
        if (state < 0) {
  ...
  auto run_no_reuse(cope::txn::handler_t& handler, int num_iter) {
    ...
    for (int iter{}; iter < num_iter; ++iter) {
      auto msg = txn::make_start_txn(cope::msg::make_noop(), txn::state_t{ iter });
      auto out = handler.send_msg(std::move(msg));
    }
  ...
  auto run_with_reuse(cope::txn::handler_t& handler, int num_iter) {
    using namespace std::chrono;
    auto start = high_resolution_clock::now();
    auto start_txn = txn::make_start_txn(cope::msg::make_noop(), txn::state_t{ 0 });
    for (int iter{}; iter < num_iter; ++iter) {
      auto out_ptr = handler.send_msg(std::move(start_txn));
